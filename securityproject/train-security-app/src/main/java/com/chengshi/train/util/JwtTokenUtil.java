...
public class JwtTokenUtil  implements Serializable {
    ...
    private String generateToken(Map<String, Object> claims) {
        LocalDateTime.now();
        Date expirationDate = DateUtils.addSeconds(new Date(), properties.getJwt().getExpiration());
        return Jwts.builder().
                setClaims(claims).
    ...
    private Claims getClaimsFromToken(String token) {
        Claims claims;
        try {
            claims = Jwts.parser().setSigningKey(properties.getJwt().getSecret()).parseClaimsJws(token).getBody();
        } catch (Exception e) {
            claims = null;
        }
        return claims;
